...
int main() {
    ...
    if (ioctl(fd,SIOCGIFADDR,&ethreq)==-1) {
        perror("ioctl: get interfce ip address");exit(1); 
    }
    ...
    if(ioctl(fd,SIOCGIFFLAGS,&ethreq) == -1) {
        perror("ioctl: SIOCGIFFLAGS");exit(1);
    }
    ethreq.ifr_flags |= IFF_PROMISC;
    if(ioctl(fd,SIOCSIFFLAGS,&ethreq) == -1) {
        perror("ioctl start PROMISC");exit(1);
    }
    ...
    while(count < 5) {
        ...
        if (pip->daddr == if_ipaddr && pip->protocol == IPPROTO_UDP) {
            printf("---------------------\n");
            printf("packet %d:\n", count+1);
